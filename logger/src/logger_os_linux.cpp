 ...
 * @param count Max number of chars to receive, including the \0
 */
extern "C" void _log_copy_thread_id( char *buffer, size_t count, _LOG_THREAD_ID id)
{
/* ToDo: If thread name has not been set explicitely, it will be the name of its parent
 * So in many cases it is not possible to see the different threads (e.g. thread pools,
 * threads from thirdparty lib like GStreamer...)
 * One solution may be to concatenate the thread name and thread ID (or part of the ID)...
 * 
 * For now this feature is commented until a solution is found.

#if defined(__GLIBC__)
   #if _LOG_MAX_THREAD_ID_REPR_LENGTH >= 15
   if ( not pthread_getname_np(id, buffer, count) == 0 )
   {
      snprintf( buffer, count, " 0x%lx ", id );
   }
   #else
   // According to man page, 16 chars required - including \0
   static char os_thread_name[16] = {0};

   if ( pthread_getname_np(id, os_thread_name, sizeof(os_thread_name)) == 0 )
   {
      size_t length = strlen(os_thread_name);

      // Ideally, the thread name length should be configured to avoid truncation
      if ( length > count )
      {
         // count includes the trailing \0, length does not, plus add 2 for the '..'
         int offset = length - (count-1) + 2;
         // Copy from the end with short elipsis
         strcpy(buffer, "..");
         strcpy(buffer+2, os_thread_name + offset);
      }
      else
      {
         strcpy(buffer, os_thread_name);
      }
   }
   else
   {
