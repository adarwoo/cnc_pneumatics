 ...
 * @param[in] nbSlotsRequested   The number of free slots requested in message queue for logging
 ...
 */
static int _log_mqueue_push(
   logMessageOutput_t output,
   size_t nbSlotsRequested,
   const char * msgStr,
   void * param)
{
   int ret = 0;
   ...
   if ((_log_mqueue_ptr->nb_of_msgs + nbSlotsRequested - 1) >= _log_mqueue_ptr->depth)
   {
   ...
   else
   {
      ...
      strncpy(_log_mqueue_ptr->next_msg_ptr->buffer, msgStr, _LOG_SIZED_FOR(_LOG_MAX_TRACE) - 1);
...
static void _log_thread_stop(void)
{
   ...
   if (_log_thread_is_initialized && !_thread_is_stopped)
   {
      ...
      _log_mqueue_push(LOG_MSG_OUT_DUMMY, 1, "STOP", NULL);
...
static int _push_log_to_queue(logMessageOutput_t output, const char * msgStr, void * param)
{
   ...
   if (nb_dropped_msg > 0)
   {
      ...
      snprintf ( buffer, sizeof(buffer), "##### Dropped logs: %zu #####", nb_dropped_msg );
      ...
      /*
       * Push info message into queue.
       * Reserve 2 slots in order to be sure that the new message could be pushed after
       */
      if (_log_mqueue_push(output, 2, buffer, errorParam) == 0)
      {
         /* Info message successfully pushed */

         /* Push new log message */
         if (_log_mqueue_push(output, 1, msgStr, param) == 0 )
         {
            /* Log message pushed successfully: Erase counter */
            nb_dropped_msg = 0;
            ret = 0;
         }
         else
      ...
      {
         /* Info message cannot be pushed: consider that the new message is dropped too */
         ++nb_dropped_msg;
      }
   }
   else
