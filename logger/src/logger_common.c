...
#  endif
   ...
   for (;;)
   {
      ...
      else if ( strncmp( env_log, "dom[", 4 ) == 0 )
      {
         ...
         logLevel_t domLevel = LOG_LEVEL_UNKNOWN;
      ...
      // The keyword is not recognized by generic logger, allow the extended init to process it
      else if ( _log_ext_init(env_log) )
      {
         // Nothing to do here. The keyword has been accepted in the platform specific init.
      }
#ifndef LOGGER_SMALL
      else
      {
...
// Provide 'weak' - overridable function for gcc compilers
// These can be overwritten in the specific implementations
#ifdef __GNUC__
   __attribute__((weak))
#endif   
    void _log_abort()
   ...
   {
      // Let platforms do the implementation
   }

/**
 * This function can be overwritten in platform specific implementation.
 * If the generic logger doesn't understand a specific keyword from the
 * configuration string, it will pass the keywork to ext_init.
 */
#ifdef __GNUC__
   __attribute__((weak))
#endif 
   bool _log_ext_init( const char *keyword )
   {
      // Let platforms do the implementation
      (void)keyword;
      return false;
