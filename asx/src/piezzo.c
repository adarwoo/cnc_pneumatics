...
reactor_handle_t piezzo_reactor;
...
void _play_next_note(void *arg)
{
   ...
   if ( *(piezzo.next_note) != '\0' )
   {
      ...
      if ( last_tc_value != new_tc_value || piezzo.slur == false )
      {
         ...
         {
            // Shift the reference value by the number of octaves
      PIEZZO_TCB.CNT = 0;
      ...
      // Re-arm the timer
      timer_instance = timer_arm(piezzo_reactor, timer_get_count_from_now(piezzo.duration), 0, NULL);
      }
      else
      {
...
void piezzo_init(void)
{
   ...
   piezzo_reactor = reactor_register(_play_next_note, PIEZZO_PRIO, 1);
}
...
bool piezzo_is_playing(void)
{
   return timer_instance != TIMER_INVALID_INSTANCE;
}
